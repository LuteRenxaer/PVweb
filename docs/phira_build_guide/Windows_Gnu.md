...
2. 在命令提示符（cmd）或者 PowerShell 使用 `cargo build --release -p Phira-Vrenxz-main` ，如果您在编译过去的版本，在
...
3. 在命令提示符（cmd）或者 PowerShell 使用 `cargo build --target=i686-pc-windows-gnu --release --package Phira-Vrenxz-main`
   ，如果不出意外，在 `openssl-sys(build)` 时，您将卡很久很久，请不要退出，这是正常的。
4. 构建完成后，在 `.\target\release\` 目录下您可以找到编译完成的主程序
5. 复制 `.\assets\` 目录中的所有文件到 `.\target\release\assets\` ，至此，构建流程全部完成，您可以直接运行 `Phira-Vrenxz-main.exe`
   检查资源文件是否完整。
